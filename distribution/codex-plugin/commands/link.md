...
1. Identify the target task from: "$ARGUMENTS". If it is a 24-hex task id, use it
   directly. Otherwise find it: `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/blitzit/blitzit/1.1.0/runtime/blitzit.mjs" tasks --search "<words>" --json` (or
   `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/blitzit/blitzit/1.1.0/runtime/blitzit.mjs" today --cached`), and pick the best match — ask only if genuinely
   ambiguous.
2. Run `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/blitzit/blitzit/1.1.0/runtime/blitzit.mjs" link <taskId>`. The current conversation is auto-detected (the
...
To stop tracking: `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/blitzit/blitzit/1.1.0/runtime/blitzit.mjs" unlink` (banks the time so far, then detaches).
