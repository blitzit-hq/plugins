...
Steps:
1. If the user did not name a target list, run `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/blitzit/blitzit/1.1.0/runtime/blitzit.mjs" lists --cached` and pick the
   most relevant list (ask only if it is genuinely ambiguous).
2. Create it with `node "${CODEX_HOME:-$HOME/.codex}/plugins/cache/blitzit/blitzit/1.1.0/runtime/blitzit.mjs" new "<title>" --list <listId>`, adding `--board today`
