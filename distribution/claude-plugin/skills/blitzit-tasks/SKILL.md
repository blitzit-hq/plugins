...
```bash
node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" goal "<concise goal>"
node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" subtask list
node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" subtask add "Investigate the current behavior" "Implement the user-visible fix"
node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" subtask replace 2 "Validate and hand off the completed fix"
node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" subtask done 2
node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" subtask rm 3
```
...
3. Mark a milestone done only when it genuinely landed; never batch-fill a plan
   after the fact.
4. When related work changes shape, use `node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" subtask replace <old> "<new>"`.
...
Do not create a separate task for this plan. `node "${CLAUDE_PLUGIN_ROOT}/runtime/blitzit.mjs" new` is for work the user
